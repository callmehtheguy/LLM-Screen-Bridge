...
DO_NOT_TOUCH = {
    ...
    "PROMPTS": {
        "STEP_1_MAP": "Send an exact replica of what the screenshot says in your own format - text, table, etc. Also map out every button, number, and text area. List contents and 0-1000 coordinates. If this is a problem/question that needs to be anwered, show full working and thought process after the mappings. Do not round mid calculation, do not get anything wrong. Also it tends to be a bit picky, for example bearings have to be 3 digits, and costs have cant be £1.3 they have to be £1.30. If there is no visible way to input an answer, try clicking on something saying 'enter number' or similar. If this is the case, only click on ONE of those buttons, if you click on multiple, it will not work.",
        "STEP_2_SOLVE": (
